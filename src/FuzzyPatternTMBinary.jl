...
mutable struct TATeam <: AbstractTATeam
    ...
    function TATeam(clause_size::Int64, clauses_num::Int64, include_limit::Int64, state_min::Int64, state_max::Int64)
        positive_clauses = fill(UInt8(include_limit - 1), clause_size, clauses_num)
        negative_clauses = fill(UInt8(include_limit - 1), clause_size, clauses_num)
        positive_clauses_inverted = fill(UInt8(include_limit - 1), clause_size, clauses_num)
        negative_clauses_inverted = fill(UInt8(include_limit - 1), clause_size, clauses_num)
        positive_included_literals = [UInt16[] for _ in 1:clauses_num]
        negative_included_literals = [UInt16[] for _ in 1:clauses_num]
        positive_included_literals_inverted = [UInt16[] for _ in 1:clauses_num]
        negative_included_literals_inverted = [UInt16[] for _ in 1:clauses_num]
...
mutable struct TMClassifier{ClassType} <: AbstractTMClassifier
    ...
    function TMClassifier{ClassType}(clauses_num::Int64, T::Int64, S::Int64; states_num::Int64=256, include_limit::Int64=128, L::Int64=16, LF::Int64=4) where ClassType
        return new{ClassType}(clauses_num, T, S, 0, L, LF, include_limit, typemin(UInt8), states_num - 1, nothing)
    end
end
...
mutable struct TMClassifierCompiled{ClassType} <: AbstractTMClassifier
    const clauses_num::Int64
    const T::Int64
    const S::Int64
    const s::Int64
    const L::Int64
    const LF::Int64
    clauses::Union{TATeamCompiled, Nothing}

    function TMClassifierCompiled{ClassType}(clauses_num::Int64, T::Int64, S::Int64, s::Int64, L::Int64, LF::Int64) where ClassType
        return new{ClassType}(clauses_num, T, S, s, L, LF, nothing)
    end
end
...
function initialize!(tm::TMClassifier, X::Vector{TMInput}, Y::Vector)
    ...
    tm.clauses = TATeam(length(first(X)), tm.clauses_num, tm.include_limit, tm.state_min, tm.state_max)
end
...
function vote(ta::AbstractTATeam, x::TMInputBatch, LF::Int64, votes_pos::Vector{Int64}, votes_neg::Vector{Int64}, b::Vector{Int64})::Tuple{Vector{Int64}, Vector{Int64}}
    @inbounds for (pil, piil) in zip(ta.positive_included_literals, ta.positive_included_literals_inverted)
        check_clause(x, pil, piil, LF, b)
        @inbounds @simd for i in 1:64
            votes_pos[i] += b[i]
        end
    end
    @inbounds for (nil, niil) in zip(ta.negative_included_literals, ta.negative_included_literals_inverted)
...
function predict(tm::AbstractTMClassifier, x::AbstractTMInput)::Bool
    pos, neg = vote(tm.clauses, x, tm.LF)
    return pos > neg
end
...
# end
function predefine_batch_arrays(tm::AbstractTMClassifier)::Tuple{Vector{Int64}, Vector{Int64}, Vector{Int64}, Vector{Int64}}
    best_vote::Vector{Int64} = fill(typemin(Int64), 64)
    votes_pos::Vector{Int64} = Vector{Int64}(undef, 64)
    votes_neg::Vector{Int64} = Vector{Int64}(undef, 64)
    b::Vector{Int64} = Vector{Int64}(undef, 64)
    return best_vote, votes_pos, votes_neg, b
end


#function predict(tm::AbstractTMClassifier, x::TMInputBatch, best_vote::Vector{Int64}, best_cls::Vector, votes::Vector{Int64})
function predict(tm::AbstractTMClassifier, x::TMInputBatch, best_vote::Vector{Int64}, votes_pos::Vector{Int64}, votes_neg::Vector{Int64}, b::Vector{Int64})
    best_cls = Vector{class_type(tm)}(undef, x.batch_size)
    fill!(votes_pos, 0)
    fill!(votes_neg, 0)
    pos, neg = vote(tm.clauses, x, tm.LF, votes_pos, votes_neg, b)
    @inbounds for i in 1:x.batch_size
        best_cls[i] = pos[i] > neg[i]
    end
    return best_cls
end
...
#    thread_best_cls = Vector{Vector{class_type(tm)}}(undef, Threads.nthreads())
    thread_votes_pos = Vector{Vector{Int64}}(undef, Threads.nthreads())
    thread_votes_neg = Vector{Vector{Int64}}(undef, Threads.nthreads())
...
#        thread_best_vote[tid], thread_best_cls[tid], thread_votes[tid] = predefine_batch_arrays(tm)
        thread_best_vote[tid], thread_votes_pos[tid], thread_votes_neg[tid], thread_b[tid] = predefine_batch_arrays(tm)
    end
    @threads for i in eachindex(X)
        tid::Int64 = Threads.threadid()
#        predicted[i] = predict(tm, X[i], thread_best_vote[tid], thread_best_cls[tid], thread_votes[tid])
        predicted[i] = predict(tm, X[i], thread_best_vote[tid], thread_votes_pos[tid], thread_votes_neg[tid], thread_b[tid])
    end
    return predicted
end
...
function train!(tm::TMClassifier, x::TMInput, y::Any; shuffle::Bool=true)
    if y == true
        feedback!(tm, tm.clauses, x, tm.clauses.positive_clauses, tm.clauses.positive_clauses_inverted, tm.clauses.negative_clauses, tm.clauses.negative_clauses_inverted, tm.clauses.positive_included_literals, tm.clauses.positive_included_literals_inverted, tm.clauses.negative_included_literals, tm.clauses.negative_included_literals_inverted, true)
    else
        feedback!(tm, tm.clauses, x, tm.clauses.negative_clauses, tm.clauses.negative_clauses_inverted, tm.clauses.positive_clauses, tm.clauses.positive_clauses_inverted, tm.clauses.negative_included_literals, tm.clauses.negative_included_literals_inverted, tm.clauses.positive_included_literals, tm.clauses.positive_included_literals_inverted, false)
    end
end
...
function compile(tm::TMClassifier; verbose::Int=0)::TMClassifierCompiled
    if verbose > 0
        ...
        tmc.clauses = TATeamCompiled(tm.clauses_num)
        ta = tm.clauses
        ...
        for (j, (c, ci)) in enumerate(zip(eachcol(ta.negative_clauses), eachcol(ta.negative_clauses_inverted)))
            tmc.clauses.negative_included_literals[j] = [i for i = 1:ta.clause_size if c[i] >= ta.include_limit]
            tmc.clauses.negative_included_literals_inverted[j] = [i for i = 1:ta.clause_size if ci[i] >= ta.include_limit]
            if verbose > 0
                append!(neg, length(tmc.clauses.negative_included_literals[j]))
                append!(neg_inv, length(tmc.clauses.negative_included_literals_inverted[j]))
            end
        end
    end
    if verbose > 0
...
function diff_count(tm::AbstractTMClassifier)::Tuple{Int64, Int64, Int64, Int64, Int64}
    ...
    neg = []
    neg_inv = []
    for c in tm.clauses.positive_included_literals
        append!(pos, [c])
    end
    for c in tm.clauses.negative_included_literals
        append!(neg, [c])
    end
    for c in tm.clauses.positive_included_literals_inverted
        append!(pos_inv, [c])
    end
    for c in tm.clauses.negative_included_literals_inverted
        append!(neg_inv, [c])
    end
    pos = vcat(pos...)
    neg = vcat(neg...)
