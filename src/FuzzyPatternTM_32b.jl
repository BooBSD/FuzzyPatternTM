...
mutable struct TATeam <: AbstractTATeam
    ...
    positive_included_literals::Vector{Vector{UInt32}}
    negative_included_literals::Vector{Vector{UInt32}}
    positive_included_literals_inverted::Vector{Vector{UInt32}}
    negative_included_literals_inverted::Vector{Vector{UInt32}}
    ...
    function TATeam(clause_size::Int64, clauses_num::Int64, include_limit::Int64, state_min::Int64, state_max::Int64)
        ...
        positive_included_literals = [UInt32[] for _ in 1:floor(Int, clauses_num / 2)]
        negative_included_literals = [UInt32[] for _ in 1:floor(Int, clauses_num / 2)]
        positive_included_literals_inverted = [UInt32[] for _ in 1:floor(Int, clauses_num / 2)]
        negative_included_literals_inverted = [UInt32[] for _ in 1:floor(Int, clauses_num / 2)]
...
function check_clause(x::TMInput, literals::Vector{UInt32}, literals_inverted::Vector{UInt32}, LF::Int64)::Int64
...
function check_clause(x::TMInputBatch, literals::Vector{UInt32}, literals_inverted::Vector{UInt32}, LF::Int64, b::Vector{Int64})#::Vector{Int64}
    length_literals::Int64 = (length(literals) + length(literals_inverted))
    c::Int64 = 0 < length_literals < LF ? length_literals : LF
...
function merge!(new_tm::TMClassifier, tms::TMClassifier...; algo::Symbol=:merge)
    ...
    @threads for cls in collect(keys(new_tm.clauses))
        ...
        elseif algo == :join
            ...
            new_tm.clauses[cls].positive_included_literals = [UInt32[] for _ in 1:floor(Int, clauses_num_half)]
            new_tm.clauses[cls].negative_included_literals = [UInt32[] for _ in 1:floor(Int, clauses_num_half)]
            new_tm.clauses[cls].positive_included_literals_inverted = [UInt32[] for _ in 1:floor(Int, clauses_num_half)]
            new_tm.clauses[cls].negative_included_literals_inverted = [UInt32[] for _ in 1:floor(Int, clauses_num_half)]
        end
