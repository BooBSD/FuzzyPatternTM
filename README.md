...
Additional experiments on the *Fashion-MNIST* dataset with convolutional preprocessing yield similar results: **92.07%** test accuracy with only **2 clauses per class**, **93.06%** with 20 clauses per class, and **94.68%** with 8000 clauses per class. This demonstrates an approximate **400× reduction** in clause count compared to Composite TM, whose best performance was **93.00%** accuracy using a model with **8000** clauses per class. Finally, on the noisy *Amazon Sales* dataset, FPTM outperforms both the latest Graph TM approach and the Graph Convolutional Neural Network, achieving **85.19%** test accuracy under **20% noise**, compared to 78.17% and 66.23%, respectively.
